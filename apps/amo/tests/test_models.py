...
def test_skip_cache():
    assert getattr(context._locals, 'skip_cache', False) == False
    with context.skip_cache():
        assert context._locals.skip_cache == True
        with context.skip_cache():
            assert context._locals.skip_cache == True
        assert context._locals.skip_cache == True
    assert context._locals.skip_cache == False


def test_use_master():
    local = context.multidb.pinning._locals
    assert getattr(local, 'pinned', False) == False
    with context.use_master():
        assert local.pinned == True
        with context.use_master():
            assert local.pinned == True
        assert local.pinned == True
    assert local.pinned == False
...
class TestModelBase(TestCase):
    ...
    def test_change_called_on_new_instance_save(self):
        for create_addon in (Addon, Addon.objects.create):
            ...
            assert kw['old_attr']['site_specific'] == False
            assert kw['new_attr']['site_specific'] == True
    ...
    def test_change_called_on_update(self):
        ...
        assert self.cb.called
        kw = self.cb.call_args[1]
        assert kw['old_attr']['site_specific'] == True
        assert kw['new_attr']['site_specific'] == False
        assert kw['instance'].id == addon.id
        assert kw['sender'] == Addon
