...
class TestESExtensions(ExtensionTestCase):
    ...
    def test_landing(self):
        ...
        doc = pq(r.content)
        assert doc('body').hasClass('s-featured') == True
...
class TestFeaturedLocale(amo.tests.TestCase):
    ...
    def test_featured_duplicated(self):
        ...
        assert Addon.featured_random(amo.FIREFOX, 'en-US').count(1003) == 1
...
class TestSearchToolsPages(BaseSearchToolsTest):
    ...
    def test_sidebar_extensions_links(self):
        ...
        assert (
            sorted([a.text.strip() for a in links]),
    ...
    def test_no_featured_addons_by_category(self):
        ...
        assert response.context['filter'].field == 'popular'
        ...
        sort_links = [urlparse(a.attrib['href']).path for a in
                      doc('.listing-header ul li a')]
        assert (
            set(sort_links), set([reverse('browse.search-tools',
            ==                               args=('bookmarks',))]))
    ...
    def test_rss_links_per_page(self):
        ...
        assert (
            get_link(reverse('browse.search-tools')),
            == reverse('browse.search-tools.rss') + '?sort=popular')

        assert (
            get_link(reverse('browse.search-tools') + '?sort=name'),
            == reverse('browse.search-tools.rss') + '?sort=name')

        assert get_link(reverse('browse.search-tools' == args=('bookmarks',)),
...
class TestSearchToolsFeed(BaseSearchToolsTest):
    ...
    def test_created_search_tools(self):
        ...
        assert (
            [e.text for e in doc('rss channel item title')],
    ...
    def test_search_tools_within_a_category(self):
        ...
        assert (
            doc('rss channel description')[0].text,
...
class TestCategoriesFeed(amo.tests.TestCase):
    ...
    def test_title(self):
        assert (
            self.feed.title(self.category),
            == u'%s :: Add-ons for %s' % (self.wut, self.u))

    def test_item_title(self):
        assert (
            self.feed.item_title(self.addon),
...
class TestPersonas(amo.tests.TestCase):
    ...
    def test_personas_grid(self):
        ...
        assert r.status_code == 200
        assert r.context['is_homepage'] == True
